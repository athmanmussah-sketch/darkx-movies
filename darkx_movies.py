...
# ========== CONFIGURATION ==========
CONFIG_FILE = os.path.join(Path.home(), '.darkx_movies_config.json')
DEFAULT_DOWNLOAD_FOLDER = "/storage/emulated/0/Download/Movies"
...
def load_config():
    ...
    return {
        ...
        'default_quality': 'bestvideo[height<=1080]+bestaudio/best[height<=1080]',
        'default_format': 'mp4',
        'history_size': 10,
        'use_aria2': True,          # Use aria2 for faster downloads
        'watermark': False,          # Optional watermark
        'subtitle_lang': 'en'
    }
...
║      ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝  ╚═╝                 ║
║                                                              ║
║                    DARKX MOVIES DOWNLOADER                   ║
║                         VERSION 1.0.0                        ║
╠══════════════════════════════════════════════════════════════╣
║                                                              ║
║  Developer: DarkX Dev                      Contact: 255775710774  ║
║  Features: Fast downloads (aria2), 4K/1080p, Playlists,     ║
║            Subtitles, History, Config, Resume support       ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
"""
...
def check_dependencies():
    ...
    try:
        ...
        deps_ok = False
        print_colored("[!] ffmpeg not found (required for merging)", 'yellow')
    
    # Check for aria2 (optional but recommended for speed)
    try:
        subprocess.run(['aria2c', '--version'], capture_output=True, check=True)
        print_colored("[✓] aria2 found (fast downloads enabled)", 'green')
    except:
        print_colored("[!] aria2 not found (install for faster downloads)", 'yellow')
        if config.get('use_aria2', True):
            config['use_aria2'] = False
            save_config(config)
...
def install_dependencies():
    ...
    try:
        subprocess.run(['pkg', 'install', 'python', 'ffmpeg', 'aria2', '-y'], check=True)
        subprocess.run(['pip', 'install', '--upgrade', 'yt-dlp', 'tqdm', 'colorama'], check=True)
        print_colored("[✓] Dependencies installed successfully!", 'green')
    except:
        print_colored("[!] Failed to install dependencies", 'red')
        sys.exit(1)

def check_disk_space(url, min_free_mb=500):
    """Check if there's enough free space (approx) - movies need more space."""
    try:
...
def apply_watermark(filepath):
    """Optional watermark for movies - can be enabled in settings."""
    if not config.get('watermark', False):
        return filepath
    ...
    if ext in ['.mp4', '.mkv', '.avi', '.mov']:
        # Video watermark
        temp_file = filepath + ".temp" + ext
        cmd = [
            'ffmpeg', '-i', filepath,
            '-vf', "drawtext=text='DarkX Movies':fontcolor=white:fontsize=24:x=w-tw-10:y=10:bordercolor=black:borderw=2",
            '-codec:a', 'copy',
            '-y', temp_file
        ]
        try:
            ...
            print_colored("[✓] Watermark added", 'green')
        except:
        ...
        new_basename = f"{name} [DarkX Movies]{ext}"
        new_path = os.path.join(dirname, new_basename)
        counter = 1
        while os.path.exists(new_path):
            new_basename = f"{name} [DarkX Movies {counter}]{ext}"
            new_path = os.path.join(dirname, new_basename)
            counter += 1
        os.rename(filepath, new_path)
        print_colored(f"[✓] Audio filename updated", 'green')
        return new_path
    return filepath

# ========== DOWNLOAD CORE ==========
def download_media(url, quality, format_choice, subtitles=False, subtitle_lang='en'):
    """Main download function with aria2 support for speed."""
    ...
    print_colored(f"\n╔══════════════════════════════════════════════════════════════╗", 'magenta', bold=True)
    print_colored(f"║                     DOWNLOADING MOVIE...                       ║", 'magenta', bold=True)
    ...
    if success:
        print_colored(f"\n[✓] Movie downloaded successfully!", 'green')
        ...
        for filepath in file_paths:
            if os.path.exists(filepath):
                final_path = apply_watermark(filepath)
                add_to_history(final_path)
    else:
...
def quality_menu():
    ...
    print_colored(" 2. 1080p Full HD (Recommended)", 'yellow')
    print_colored(" 3. 720p HD", 'yellow')
    print_colored(" 4. 480p (Mobile)", 'yellow')
    print_colored(" 5. Audio only (MP3)", 'yellow')
    print_colored(" 6. Custom resolution (e.g., 720)", 'yellow')
    print_colored(" 7. Best quality (no limit)", 'yellow')
    ...
    if choice == '6':
        ...
        if custom.isdigit():
            ...
            print_colored("Invalid input, using 1080p.", 'red')
            return quality_map['2']
    
    return quality_map.get(choice, quality_map['2'])

def format_menu():
    print_colored("\n╔══════════════════════════════════════════════════════════════╗", 'cyan', bold=True)
    print_colored("║                     FORMAT OPTIONS                            ║", 'cyan', bold=True)
    print_colored("╚══════════════════════════════════════════════════════════════╝", 'cyan', bold=True)
    print_colored(" 1. MP4 Video (Recommended)", 'yellow')
    print_colored(" 2. MP3 Audio (Soundtrack)", 'yellow')
...
def handle_playlist(url):
    """Ask user how to handle playlist (movie series)."""
    try:
        result = subprocess.run(
        ...
        if result.returncode == 0 and result.stdout:
            ...
            if 'playlist_count' in data or 'playlist' in url:
                print_colored("\n╔══════════════════════════════════════════════════════════════╗", 'magenta', bold=True)
                print_colored("║                     MOVIE SERIES DETECTED                    ║", 'magenta', bold=True)
                print_colored("╚══════════════════════════════════════════════════════════════╝", 'magenta', bold=True)
                print_colored(f"Title: {data.get('playlist', 'Unknown')}", 'yellow')
                print_colored("Options:", 'cyan')
                print_colored(" 1. Download entire series", 'yellow')
                print_colored(" 2. Select specific episodes (by number)", 'yellow')
                print_colored(" 3. Cancel", 'yellow')
                choice = input("⚡ Choose (1-3): ").strip()
                ...
                if choice == '2':
                    ...
                    indices = input("Enter episode numbers (e.g., 1,3,5-10): ").strip()
                    selected = []
                    parts = indices.split(',')
                    for part in parts:
                        if '-' in part:
...
def get_user_input():
    ...
    print_colored("╚══════════════════════════════════════════════════════════════╝", 'cyan', bold=True)
    print_colored(" 1. Download movie from URL(s)", 'yellow')
    print_colored(" 2. Search for movies on YouTube", 'yellow')
    print_colored(" 3. Batch download from file (URLs list)", 'yellow')
    print_colored(" 4. Show download history", 'yellow')
    print_colored(" 5. Settings", 'yellow')
...
def settings_menu():
    ...
    while True:
        ...
        print_colored(f" 5. Use aria2 (fast download): {config.get('use_aria2', True)}", 'yellow')
        print_colored(f" 6. Add watermark: {config.get('watermark', False)}", 'yellow')
        print_colored(f" 7. Default subtitle language: {config.get('subtitle_lang', 'en')}", 'yellow')
        print_colored(" 8. Back to main menu", 'yellow')
        
        opt = input("⚡ Choose option (1-8): ").strip()
        if opt == '1':
            ...
            config['use_aria2'] = not config.get('use_aria2', True)
            save_config(config)
            print_colored(f"aria2 set to {config['use_aria2']}", 'green')
        elif opt == '6':
            config['watermark'] = not config.get('watermark', False)
            save_config(config)
            print_colored(f"Watermark set to {config['watermark']}", 'green')
        elif opt == '7':
            lang = input("Enter default subtitle language (e.g., en, sw): ").strip()
            if lang:
                config['subtitle_lang'] = lang
                save_config(config)
        elif opt == '8':
            break
        time.sleep(1)
...
def main():
    ...
    if not check_dependencies():
        ...
        if not check_dependencies():
            print_colored("[!] Installation failed. Exiting.", 'red')
            sys.exit(1)
    ...
    while True:
        ...
        if config.get('use_aria2', False) and shutil.which('aria2c'):
            print_colored("⚡ Fast download mode: ENABLED (aria2)", 'yellow')
        choice = get_user_input()
        
        if choice == '1':
            urls_input = input("\nEnter movie URL(s) (separate with space): ").strip().split()
            if not urls_input:
                continue
            ...
            query = input("Enter movie title or search term: ").strip()
            if not query:
                continue
            ...
            print_colored("\nThank you for using DarkX Movies Downloader!", 'magenta', bold=True)
